...
export function TourBookingsPanel() {
  ...
  async function loadBookings() {
    setLoading(true)
    try {
      const res = await fetch('/api/tour-bookings', { cache: 'no-store' })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || 'Failed to load')
      setBookings(data.bookings || [])
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Failed to load tour bookings')
    } finally {
      setLoading(false)
    }
  }
  ...
  const filtered = sourceFilter
    ? bookings.filter((b) => (b.source || 'website') === sourceFilter)
    : bookings
  ...
  return (
    <div>
      ...
      <div style={{ ...card, marginBottom: 20, display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        {['', 'website', 'manual', 'internal'].map((s) => (
          <button
            key={s || 'all'}
            ...
            style={{
              padding: '6px 14px',
              borderRadius: 20,
              border: `1px solid ${sourceFilter === s ? 'rgba(184,149,106,0.4)' : 'rgba(240,236,228,0.12)'}`,
              background: sourceFilter === s ? 'rgba(184,149,106,0.12)' : 'transparent',
              color: sourceFilter === s ? '#b8956a' : muted,
              cursor: 'pointer',
              fontSize: 12,
              fontFamily: "'Barlow', sans-serif",
            }}
          >
            {s === '' ? 'All' : s === 'website' ? 'Website' : s === 'manual' ? 'Manual' : 'Internal'}
          </button>
        ))}
      </div>
      ...
      <div style={card}>
        {loading ? (
          <div style={{ color: muted, padding: 12 }}>Loading tour bookings...</div>
        ) : filtered.length === 0 ? (
          <div style={{ color: muted, padding: 24, textAlign: 'center' }}>No tour bookings yet</div>
        ) : (
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 860 }}>
              <thead>
                <tr style={{ borderBottom: '1px solid rgba(240,236,228,0.1)' }}>
                  {['Reference', 'Source', 'Customer', 'Tour', 'Date', 'Guests', 'Vehicle', 'Created By', 'Status', ''].map((h) => (
              ...
              <tbody>
                {filtered.map((b) => (
                  <tr key={b.id} style={{ borderBottom: '1px solid rgba(240,236,228,0.06)' }}>
                    ...
                    <td style={{ padding: '10px 12px' }}><SourceBadge source={b.source} /></td>
                    ...
                    <td style={{ padding: '10px 12px', fontSize: 13, color: muted }}>{b.vehicle_name || '—'}</td>
                    <td style={{ padding: '10px 12px' }}>
                      {b.created_by_name ? (
                        <UserColorBadge name={b.created_by_name} color={b.created_by_color} />
                      ) : (
                        <span style={{ fontSize: 11, color: muted, letterSpacing: '0.04em' }}>Website</span>
                      )}
                    </td>
                    <td style={{ padding: '10px 12px' }}><StatusBadge status={b.status} /></td>
                    <td style={{ padding: '10px 12px' }}>
                      {b.status !== 'cancelled' && b.source !== 'website' && (
